...
The goal of `cobaltResume` is to create professionally-looking résumés and cover letters in RStudio. LaTeX code has been written in the `cobaltResume.cls`, `cobaltCoverLetter.cls`, `cobaltResume.tex`, and `cobaltCoverLetter.tex` files to format the résumé and cover letter files.  I have written my own résumé and cover letter using `cobaltResume`, which can be viewed [my resume](https://github.com/sebsciarra/cobaltResume/blob/main/man/resume_cover_letter.pdf). Note that the cover letter is populated with placeholder content.  In the sections that follow, I will explain the following four components to using `cobaltResume`: 
...
The goal of `cobaltResume` is to create professionally-looking résumés and cover letters. Here is a résumé and cover letter I created for myself using  `cobaltResume`: 
...
In the `resume.Rmd` file, a résumé can be completed by using three features. First, information related to identification and skills can be included in the résumé by filling in the appropriate YAML header variables shown below.
...
In the `cover_letter.Rmd` file, a résumé can be completed by using three features. First, information related to identification and skills can be included in the résumé by filling in the appropriate YAML header variables shown below.
...
To merge the résumé and cover letter into one PDF, make sure that each `.Rmd` file exists in its own folder (as is the case when the project is first created).  With the résumé and cover letter `.Rmd` files in their own folders, they can be merged by specifying the `knit` YAML variable in either the `résumé .Rmd` and/or the `cover_letter.Rmd` file(s) such that 
